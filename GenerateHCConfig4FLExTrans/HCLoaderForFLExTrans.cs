...
;        }
		...
		private IEnumerable<Pattern<Word, ShapeNode>> LoadReduplicationPatterns(string patternStr)
		{
			foreach (string token in TokenizeContext(patternStr))
			{
				if (token.StartsWith("["))
				{
					...
					var pattern = new Pattern<Word, ShapeNode>(token.Substring(1, token.Length - 2).Trim().Replace("^",kCaretInID), new Constraint<Word, ShapeNode>(ctxt.FeatureStruct) {Tag = ctxt});
					pattern.Freeze();
		...
		private IEnumerable<MorphologicalOutputAction> LoadReduplicationOutputActions(string patternStr)
		{
			foreach (string token in TokenizeContext(patternStr))
			{
				if (token.StartsWith("["))
				{
					yield return new CopyFromInput(token.Substring(1, token.Length - 2).Trim().Replace("^",kCaretInID));
				}
				else
				{
